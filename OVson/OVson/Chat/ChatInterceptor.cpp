...
// Star renkleri (0-100 gri, 100-200 beyaz, 200-300 gold, 300-400 aqua, 400-500 yeşil, 500-600 aqua, 600-700+ kırmızı)
...
static const char *teamInitial(const std::string &team)
{
    ...
    if (team == "Pink")
        return "P";
    if (team == "Gray" || team == "Grey")
        return "G"; // Çakışma: Green ile aynı, fakat tek harf isteniyor
    return "?";
}
...
static void detectTeamsFromLine(const std::string &chat)
{
    ...
    for (const char *t : teams)
    {
        ...
        if (chat.find(needle1) != std::string::npos)
        {
            ...
            // We don't know local IGN here; skip mapping
        }
